...
import type { AmbienteSanitario } from "@/lib/calc/sanitaria_types";
import { rid } from "@/lib/id";

interface Props {
  ambientes: AmbienteSanitario[];
  setAmbientes: React.Dispatch<React.SetStateAction<AmbienteSanitario[]>>;
  plantas: number;
}

// LISTA DE ARTEFACTOS ACTUALIZADA
const ARTEFACTOS_DISPONIBLES = [
  { id: "inodoro", label: "Inodoro" },
  { id: "bidet", label: "Bidet" },
  { id: "pileta_patio", label: "Pileta de Patio" },
  { id: "banera", label: "Bañera" },
  { id: "boca_acceso", label: "Boca de Acceso" },
  { id: "pileta_cocina", label: "Pileta de Cocina" },
...
export default function Step2_Ambientes({ ambientes, setAmbientes, plantas }: Props) {
  ...
  const handleChange = (id: string, field: keyof AmbienteSanitario, value: string | number) => {
  ...
  const handleArtefactoChange = (ambId: string, artId: string, count: number) => {
    setAmbientes(prev => prev.map(a => {
      ...
      if (count > 0) newArtefactos[artId] = count;
      else delete newArtefactos[artId];
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-[var(--color-base)]">Paso 2: Define los Ambientes con Desagüe</h3>
